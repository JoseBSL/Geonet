...
null.geo=merge(null.net,geo.split.jac.long,by=c("Poll_sp1","Poll_sp2","Network"),all=TRUE)%>%droplevels()
null.geo=null.geo[!is.na(null.geo$mean),]
...
ggplot(null.geo.sub1,aes(x=jac.ord,y=clim)+geom_point()
...
adonis(pfdist.matrix ~ pfdist$clim, permutations = 9999)
...
adonis(poldist.matrix ~ pfdist$clim, permutations = 9999)
...
#plot(prop.res) #THESE ARE TERRIBLE BUT IT MAY BE DUE TO MODEL FAMILY
