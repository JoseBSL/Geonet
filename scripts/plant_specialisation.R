...
sum.links <- geonet %>%
  group_by(Reference,Network, Plant,clim) %>%
  summarise(tot=sum(Int))

min.max <- sum.links %>%
  group_by(Reference,Network) %>%
  summarise(min=min(tot), 
            max=max(tot))

comb.links <- merge(min.max,sum.links,by=c("Reference","Network"))

comb.links$std = (comb.links$tot - comb.links$min) / (comb.links$max - comb.links$min)

links.plant <- merge(comb.links,clim.dat, by=c("Reference","Network","clim"))
#links.plant$clim <- as.factor(left(links.plant$ClimateZ,1))

#model like this but with beta:
plant_brm <- brm(std ~ clim + (1|Reference/Network),
...
plant.brm_emm <- plant_brm %>%
  emmeans( ~ clim) %>% 
  gather_emmeans_draws() %>% 
  mutate(intera = paste(clim , sep = ".")) 

##compact letter differences
plant.brm_cld<-plant.brm_emm %>% 
