...
g#subset data by insect order
g_fam_sub <- subset(g_fam4, PolOrder %in% c("Hymenoptera", "Bee","Diptera", "Lepidoptera","Syrphidae","Coleoptera"))
str(g_fam_sub)

levels(as.factor(g_fam_sub_10$clim))

g_fam_sub_5=g_fam_sub %>% 
  group_by(PolFamily) %>% 
  filter(n() >= 5)

g_fam_sub_6=g_fam_sub_5 %>% 
  group_by(PolFamily) %>% 
  filter(all(c("A", "B","C","D","E") %in% clim))


levels(as.factor(g_fam_sub_6$PolFamily))
...
#set max vector size
Sys.setenv('R_MAX_VSIZE'=3200000000)
start.time <- Sys.time()
...
#GENUS 1
obs.jac.long$Poll_sp1=gsub("\\."," ",obs.jac.long$Poll_sp1)
obs.jac.long$IGenus=word(gsub("\\."," ",obs.jac.long$Poll_sp1),1)
str(obs.jac.long)
#geo.split.jac.long$IGenus=word(gsub("\\_"," ",geo.split.jac.long$IGenus),1)

obs.jac.long=merge(obs.jac.long,poll_famord[,c("IGenus","PolOrder","PolFamily")], by = "IGenus")
obs.jac.long=obs.jac.long[,1:7]
str(obs.jac.long)
colnames(obs.jac.long)=c("PGenus1","Poll_sp1","Poll_sp2",
                               "jac","Network","Order1","Family1")

#GENUS 2
obs.jac.long$IGenus=word(gsub("\\."," ",obs.jac.long$Poll_sp2),1)
#geo.split.jac.long$IGenus=word(gsub("\\_"," ",geo.split.jac.long$IGenus),1)

obs.jac.long=merge(obs.jac.long,poll_famord[,c("IGenus","PolOrder","PolFamily")], by = "IGenus")
str(obs.jac.long)
colnames(obs.jac.long)=c("PGenus2","PGenus1","Poll_sp1","Poll_sp2","jac","Network","Order1","Family1","Order2","Family2")
str(obs.jac.long)

obs.jac.long=merge(obs.jac.long,reference[,c("Reference","Latitude","Longitude","clim","Network","Connectance","ele","Size")], by = "Network")
str(obs.jac.long)
head(obs.jac.long)

#########SUBSET########
obs.jac <- obs.jac.long %>% mutate_if(is.factor,as.character)
obs.jac[obs.jac$Family1%in%c("Stenotritidae","Apidae","Andrenidae","Colletidae","Megachilidae","Melittidae","Halictidae"),c("Order1")]="Bee"
obs.jac[obs.jac$Family1%in%c("Syrphidae"),c("Order1")]="Syrphidae"

obs.jac[obs.jac$Family2%in%c("Stenotritidae","Apidae","Andrenidae","Colletidae","Megachilidae","Melittidae","Halictidae"),c("Order2")]="Bee"
obs.jac[obs.jac$Family2%in%c("Syrphidae"),c("Order2")]="Syrphidae"

obs.jac.sub1=subset(obs.jac, Order1 %in% c("Hymenoptera", "Bee","Diptera", "Lepidoptera","Syrphidae","Coleoptera"))
obs.jac.sub1=subset(obs.jac.sub1, Order2 %in% c("Hymenoptera", "Bee","Diptera", "Lepidoptera","Syrphidae","Coleoptera"))

obs.jac.sub1 <- obs.jac.sub1 %>% mutate_if(is.factor,as.character)

obs.jac.sub1$Order1=str_trim(obs.jac.sub1$Order1, side = c("both"))
obs.jac.sub1$Order2=str_trim(obs.jac.sub1$Order2, side = c("both"))
str(obs.jac.sub1)
obs.jac.sub1$Family1=stri_enc_toutf8(obs.jac.sub1$Family1)
obs.jac.sub1$Family2=stri_enc_toutf8(obs.jac.sub1$Family2)
obs.jac.sub1$Family1=str_trim(obs.jac.sub1$Family1, side = c("both"))
obs.jac.sub1$Family2=str_trim(obs.jac.sub1$Family2, side = c("both"))

obs.jac.sub1 <- obs.jac.sub1 %>% mutate_if(is.character,as.factor)

obs.jac.sub1$jac.ord=apply(obs.jac.sub1[,c("Order1","Order2")], MARGIN = 1, function(x){
  paste(sort(x), collapse = "_")
})
obs.jac.sub1$jac.fam=apply(obs.jac.sub1[,c("Family1","Family2")], MARGIN = 1, function(x){
  paste(sort(x), collapse = "_")
})
...
write.csv(obs.jac.sub1,"data/processed/geonet_jaccard.csv")
...
pairwiseAdonis::pairwise.adonis(poldist.matrix,pfdist$clim)
pairwiseAdonis::pairwise.adonis(pfdist.matrix,pfdist$clim)
