...
g.sub$clim <- substr(g.sub$ClimateZ,1,1)
g.sub$Network <- gsub("_"," ",g.sub$Network)
g.sub$Reference <- word(g.sub$Network,1)
...
##PLOT
prop.gg=ggplot(rbind.prop.plot,aes(x=`Pollinator taxa`,y=response,col=`Pollinator taxa`))+
  geom_point(aes(y=prop_links,col=animal.order),show.legend = F,size=0.5,
             position=position_jitterdodge(dodge.width=0,jitter.width = 0.7),
             alpha=0.5)+
  geom_point(aes(col=`Pollinator taxa`),
             size=2,show.legend = F)+
  geom_errorbar(aes(ymin=lower.HPD,ymax=upper.HPD,col=`Pollinator taxa`),
